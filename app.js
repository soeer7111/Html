...
// 💡 သင့်ရဲ့ ပေးထားသော Firebase Config ID များကို အသုံးပြုထားသည်
...
function loadProfileData() {
    ...
    if (user) {
        // ဖုန်းနံပါတ်ကို displayName မှာ သိမ်းထားသည်
        ...
        document.getElementById('navbar-profile-photo').src = photoURL; 
...
window.handleRegister = async () => {
    ...
    // Username ကို Email အဖြစ် ပြောင်းသုံးသည်
    ...
    try {
        ...
        // Register ပြီးတာနဲ့ Login ဝင်ပေးလိုက်သည်
        await window.auth.signInWithEmailAndPassword(email, password); 
        
        // 🚨 Username ကို profile မှာ ထည့်ပေးလိုက်ပါ
...
window.auth.onAuthStateChanged((user) => {
    if (user) {
        // Logged In ဝင်ထားရင် UI Data ဖြည့်
        const displayEmail = user.email || 'N/A';
        // displayName ကို ဖုန်းနံပါတ် သိမ်းဖို့ သုံးထားသောကြောင့် email ကိုပဲ username အဖြစ် သုံးသည်
...
window.uploadProfilePhoto = async () => {
    ...
    try {
        ...
        // 🚨 Storage Plan Upgrade မလုပ်ထားရင် ဒီမှာ Error ပြပါမယ်
        messageDiv.textContent = `Upload Failed: ${error.message}. (Storage Plan ကို စစ်ဆေးပါ။)`;
...
window.updateProfileDetails = async () => {
    ...
    try {
        // displayName ကို ဖုန်းနံပါတ် သိမ်းဖို့ အသုံးပြုသည်
        await user.updateProfile({
            displayName: phone 
        });
...
// =================================================
// 🚨 Part 5: Video Player & Data Persistence Logic (Like Fix)
// =================================================
...
function initializeVideoPlayer() {
    ...
    // Home Page ဖွင့်တိုင်း လက်ရှိ Index ကို ပြန် Load လုပ်သည်
...
// 🚨 Like Functions (Fix: innerHTML တွင် span tag ကို ပြန်ထည့်ပြီး Status ကို Update လုပ်သည်)
function updateLikeStatus(video) {
    const likeButton = document.getElementById('like-button');
    if (!likeButton) return;

    if (video.userLiked) {
        // Like လုပ်ထားရင် Heart Emoji ပြရန်
        likeButton.innerHTML = `❤️ လိုက်ခ် (<span id="like-count">${video.currentLikes}</span>)`;
    } else {
        // Like မလုပ်ရသေးရင် Thumb-up Emoji ပြရန်
...
window.toggleLike = () => {
    ...
    // 🚨 အပြောင်းအလဲတိုင်းကို Local Storage တွင် ချက်ချင်းသိမ်းဆည်းပါ
    saveDataToStorage(); 
