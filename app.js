...
// 💡 Helper: Firestore တွင် User Data သိမ်းခြင်း (လိုပါက)
...
window.handleRegister = async () => {
    ...
    try {
        ...
        // 🚀 ပြင်ဆင်ချက်: Page မတက်သော ပြဿနာ ဖြေရှင်းရန်
...
// 4. Auth State Check Logic (Persistence Logic)
window.auth.onAuthStateChanged((user) => {
    ...
    if (user) {
        ...
        if (hash && hash !== 'login-page' && hash !== 'register-page') {
            ...
            // Hash မရှိမှသာ Home ကို ပို့ပါ၊ ရှိနေရင် onAuthStateChanged ထပ်ခေါ်မနေတော့ပါ
            showPage('home-page');
        }
...
window.uploadProfilePhoto = async () => {
    ...
    messageDiv.textContent = '❌ Photo Upload ဝန်ဆောင်မှု ယာယီ ပိတ်ထားပါသည်။ (Billing လိုအပ်ပါသည်)';
};
...
// =================================================
// 🚨 Part 5: Global Chatbox Functionality (Admin Delete Logic ပါဝင်)
// =================================================
...
window.sendMessage = async () => {
    ...
    try {
        await window.db.collection('chats').add({
            uid: user.uid,
            username: user.email, // Email အပြည့်အစုံကို သိမ်းခြင်း (Admin စစ်ဆေးဖို့)
            message: messageText,
    ...
    unsubscribeChat = window.db.collection('chats')
        ...
        .onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                // Delete Button ကို Admin ဖြစ်မှသာ ပြသခြင်း (Admin email ဖြင့် စစ်ဆေးသည်)
...
window.addComment = () => {
    ...
    const username = user.email; // Email အပြည့်အစုံကို Comment မှာ သိမ်းဆည်း
...
// =================================================
// 🚨 Part 7: Admin Panel Logic (Placeholder)
// =================================================

// Admin Page ကို ဝင်ရောက်နိုင်ခြင်း ရှိ၊ မရှိ စစ်ဆေးခြင်း
window.checkAdminStatus = () => {
    ...
    if (user && user.email === ADMIN_EMAIL) {
        if (adminMessage) adminMessage.textContent = '✅ သင်သည် Admin ဖြစ်ပါသည်။';
        if (userListDiv) loadUserLi
