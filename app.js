// =================================================
// ✅ Part 1: Firebase Configuration & Setup (FIXED)
// =================================================
...
async function saveUserDataToFirestore(user) {
    ...
    // User Data ကို ရယူပြီး registeredAt ရှိ/မရှိ စစ်ဆေးသည်
    const existingDoc = await userRef.get();
    
    // registeredAt ကို Firestore တွင် မရှိသေးမှသာ serverTimestamp ဖြင့် အစောဆုံး မှတ်သားသည်
    const registeredAtValue = existingDoc.exists && existingDoc.data().registeredAt 
                                ? existingDoc.data().registeredAt 
                                : window.firebase.firestore.FieldValue.serverTimestamp();

    await userRef.set({ 
        uid: user.uid, // UID ကို သေချာထည့်သွင်းသည်
        email: user.email, 
...
// =================================================
// ✅ Part 2: Page Navigation & UI Functions (FIXED)
// =================================================
...
function showPage(pageId) {
    ...
    if (navBar) {
        // Nav bar ကို home, profile, admin page များတွင်သာ ပြသည်
...
// =================================================
// ✅ Part 3: Authentication (Login/Register/Logout/State Check) (FIXED)
// =================================================
...
window.handleRegister = async () => {
    ...
    try {
        ...
        // FIX: result.user ကို ပို့ပြီး Firestore တွင် Data သိမ်းသည်
        await saveUserDataToFirestore(result.user); 
        
        messageDiv.textContent = 'မှတ်ပုံတင် အောင်မြင်ပါသည်။ ခဏစောင့်ပါ။'; 
        // 🚨 FIX: Delay ကို 300ms ထားပြီး Login Redirect ပြဿနာကို ဖြေရှင်းသည်
...
window.handleLogin = async () => {
    ...
    try {
        ...
        // FIX: Login ဝင်တိုင်း lastLoginAt ကို Update လုပ်သည်
        await saveUserDataToFirestore(result.user); 
...
window.auth.onAuthStateChanged((user) => {
    const navBar = document.getElementById('nav-bar');
    const adminButton = document.getElementById('admin-nav-button'); // Admin Button ကို စစ်ပါ
    ...
    if (user) {
        ...
        const hash = window.location.hash.substring(1); 
        if (hash !== 'register-page') { // Register page မှလွဲ၍ ကျန်တာ Login ကိုပို့
            showPage('login-page');
        }
        if (unsubscribeChat) unsubscribeChat(); 
        if (unsubscribeUsers) unsubscribeUsers(); 
...
// =================================================
// ✅ Part 4: Profile Page Logic & All User Update Functions (FIXED)
// =================================================
...
// =================================================
// ✅ Part 5: Global Chatbox Functionality (FIXED - UI/Error Handling)
// =================================================
    ...
    unsubscribeChat = window.db.collection('chats')
        ...
        .onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                // FIX: time ကို မှန်ကန်စွာ ပြပါ
                const time = data.timestamp ? data.timestamp.toDate().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : '...';
                ...
                // FIX: Delete Button CSS ကို ပိုမို ရိုးရှင်းစေသည်
                const deleteButtonHtml = (currentUser && currentUser.email === ADMIN_EMAIL) 
                    ? `<button onclick="window.deleteMessage('${messageId}')" style="background: none; border: none; color: #e74c3c; font-size: 10px; cursor: pointer; margin-left: 10px; padding: 0;">[X]</button>`
                    : '';
                
                // FIX: HTML Structure ကို ပိုမို ရှင်းလင်းစေသည်
...
// =================================================
// ✅ Part 6: Video Player & Data Persistence Logic (FIXED - Sidebar Highlight)
// =================================================
...
function loadVideo(video, index) {
    ...
    updateSidebarHighlight(); // Highlight ကို Update လုပ်သည်
}
