...
// 🚨 FIX 1: Admin Email ကို သတ်မှတ်ထားသည် (လက်တွေ့ Email ဖြစ်ရန် လိုအပ်သည်)
...
async function saveUserDataToFirestore(user) {
    ...
    // 🚨 FIX: displayName သည် မရှိပါက email မှ ဖြုတ်ယူထားသော username ကို သုံးသည်
    const displayName = user.displayName || user.email.split('@')[0];
    const isAdmin = user.email === ADMIN_EMAIL; // Admin ဖြစ်ကြောင်းလည်း မှတ်ထားပါ
    
    await userRef.set({ 
        uid: user.uid,
        email: user.email, 
        displayName: displayName,
        isAdmin: isAdmin, // Admin Role ကို Firestore တွင် မှတ်သည်
...
function showPage(pageId) {
    ...
    if (pageId === 'home-page') {
        ...
        window.checkAdminStatus(); // 🚨 Admin status ကို စစ်ဆေးရန် Function ကို ခေါ်သည်
    }
}
...
window.auth.onAuthStateChanged((user) => {
    ...
    if (user) {
        ...
        if (usernameDisplay) {
            // @dummy.com ကို ဖြုတ်ပြီး ပြခြင်း
...
window.deleteMessage = async (messageId) => {
    ...
    if (confirm("ဤ Chat Message ကို ဖျက်မှာ သေချာပါသလား။")) {
        try {
            // Note: Rules ကို chatMessages collection အတွက် ပြင်ဆင်ရန် လိုအပ်သည်
        ...
        .onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                const deleteButtonHtml = (currentUser && currentUser.email === ADMIN_EMAIL) 
                    ? `<button onclick="window.deleteMessage('${messageId}')" style="background: none; border: none; color: #e74c3c; font-size: 10px; cursor: pointer; float: right; margin-left: 5px; width: auto; margin-top: 0;">[X]</button>`
                    : '';
