...
function showPage(pageId) {
    ...
    if (pageId === 'home-page') {
        ...
        // Home page ရောက်ရင် Nav Bar ကို ပြပါ (Index.html ရဲ့ header ပါ)
...
window.auth.onAuthStateChanged((user) => {
    // Loading Screen မရှိလို့ ဒီ Logic ကို ဖယ်လိုက်ပါပြီ
    // const loadingPage = document.getElementById('loading-page');
    // if (loadingPage) loadingPage.style.display = 'none';

    // Nav Bar သည် Home Page header ထဲတွင် ပါဝင်နေပါသည်
    const navBar = document.getElementById('nav-bar');
    ...
    if (user) {
        ...
        // Login မဝင်ထားရင်
        if (navBar) navBar.style.display = 'none'; // မပြဖို့
        showPage('login-page');
    }
});
