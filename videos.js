...
function showPage(pageId) {
    ...
    // Home Page ကို ဝင်ရောက်မှ Video Player ကို စတင်သည်
    if (pageId === 'home-page') {
...
function setupAuthListener() {
    // window.onAuthStateChanged နဲ့ window.auth ဟာ index.html ကနေ ရောက်လာသည်
    if (window.onAuthStateChanged && window.auth) {
        window.onAuthStateChanged(window.auth, (user) => {
            if (user) {
                // User Login ဝင်ထားပါက (Session အောင်မြင်ပါက)
                const displayEmail = user.email || 'N/A';
                // Email မဟုတ်ဘဲ username ပဲ ပေးခဲ့ရင် @dummy.com ကို ဖြုတ်ပြီး ပြသည်
...
window.handleRegister = async () => {
    ...
    try {
        ...
        // ၂။ ချက်ချင်း Login ပြန်ဝင်ခိုင်းသည် (Verification ကို ကျော်လွှားရန်)
        await window.signInWithEmailAndPassword(window.auth, email, password);
        
        // Login ဝင်ပြီးတာနဲ့ onAuthStateChanged က Home ကို အလိုအလျောက် ပို့ပါစေ။
...
window.handleLogin = async () => {
    ...
    try {
        await window.signInWithEmailAndPassword(window.auth, email, password);
        // Login အောင်မြင်ရင် onAuthStateChanged က Home ကို ပို့မယ်
        messageDiv.textContent = 'Login အောင်မြင်ပါသည်။'; 

    } catch (error) {
        // Error ဖြစ်ရင် Message ကို ပြပြီး ရှင်းပစ်ရမယ်
...
// =================================================
// 🎥 Video Player Logic (အပြည့်အစုံ ပြန်ထည့်ထားသည်)
// =================================================
...
// 🚨 App စတင် run ရန်အတွက် (DOM content ပြီးမှသာ Auth Listener ကို စတင်သည်)
