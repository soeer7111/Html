...
if (window.onAuthStateChanged) {
    window.onAuthStateChanged(window.auth, (user) => {
        if (user) {
            ...
            // '@dummy.com' ကို ဖြုတ်ပြီး username သာ ပြသရန် (username/email format ကို စစ်ထားသည်)
...
window.handleRegister = async () => {
    // 🚨 inputs တွေရဲ့ ID ကို index.html ကနေ မှန်ကန်စွာ ရယူသည်
    ...
    // (သို့သော် user က email အပြည့်အစုံထည့်ထားရင်လည်း လက်ခံသည်)
    ...
    // 🚨 စောင့်ဆိုင်းနေသော စာသားကို အရင်ပြသည်
    messageDiv.textContent = 'မှတ်ပုံတင်နေပါသည်။ ကျေးဇူးပြု၍ စောင့်ဆိုင်းပါ။';

    try {
        // Firebase Function ကို ခေါ်သည်
        // 🚨 window.createUserWithEmailAndPassword ရှိမရှိ စစ်ဆေးပြီး Error ကို console တွင် ပြမည်
        ...
        if (error.code === 'auth/email-already-in-use') {
             ...
             // 🚨 အသေးစိတ် Error ကို Console တွင် ပြသည်
...
function initializeVideoPlayer() {
    // Video Player ကို တခါသာ initialize လုပ်ရန်
    if (!player) {
        player = videojs('my-video');
    }
    
    // ပထမဆုံး video ကို load ရန်
    currentVideoIndex = 0;
    renderSidebar();
...
window.addComment = () => {
    ...
    // User မရှိရင် 'Guest' အဖြစ် သတ်မှတ်သည်
...
// Initialization ကို DOMContentLoaded ပြီးနောက် စတင်ရန် (onAuthStateChanged က UI ကို ထိန်းချုပ်ပေးမည်)
